...
It's been a while since I took algebra class, so in case you, too need a refresher: a quadratic equation of the form $y = ax^2 + bx + c$ takes the shape of a parabola; the roots of the quadratic are the values of $x$ for which $y =
0$. The so-called "quadratic formula" for finding these roots (i.e., the values of $x$ where the parabola intersects the $x$ axis) is: 
$$
...
-   If there's only one root, your program should simply print the same value twice. If that root is zero, it's okay if it prints one of the zeros with a negative sign.
...
* 1 point: Code is commented adequately and variables are appropriately named
* 1 point: Correct output for $a = 0$
* 1 point: Correct output when discriminant is negative
* 1 point: Correct output when the quadratic has two distinct real roots
* 1 point: Correct output when the quadratic has only one real roots

**`representative.py` (5 points)**

* 2 points: Code uses at least one boolean logical operator: `and`, `or`, `not`
* 1 point1: Output matches exactly for someone who is eligible to run for Representative but not Senator
* 1 point: Output matches exactly for someone who is eligible for both offices
* 1 point: Output matches exactly for someone who is eligible for neither office

**Possible Deductions**

* -1 point: Author, date, and program description comment at the top of the file is missing
* -1 point: inadequate or inappropriate use of comments, or poor coding style
